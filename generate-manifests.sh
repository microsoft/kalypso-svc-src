...
for file in `find $1 -type f \( -name "*.yaml" \)`; do 
  ...
  helm template $service_name/$helm_chart --version $helm_chart_version -f $2/$service_name/$values_file_name | sed -i '/Namespace/d' > $3/$service_name/$gen_manifests_file_name
